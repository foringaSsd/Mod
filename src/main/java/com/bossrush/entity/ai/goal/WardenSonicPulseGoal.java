...
public class WardenSonicPulseGoal extends Goal {
    ...
    public void tick() {
        ...
        if (warden.getWorld() instanceof ServerWorld sw) {
            ...
            Vec3d targetPoint = target.getPos().add(0, target.getStandingEyeHeight(), 0);
            Vec3d dir = targetPoint.subtract(origin).normalize();
            for (int i = 1; i <= 12; i++) {
    ...
    private void firePulse(LivingEntity primaryTarget) {
        ...
        Vec3d origin = warden.getPos().add(0, warden.getStandingEyeHeight(), 0);
        // Aim eye-to-eye, not feet-to-feet — the old version fired from
        // the warden's (tall) eye height toward the target's ground
        // position, sending the beam over the target's head instead of
        // through it.
        Vec3d targetPoint = primaryTarget.getPos().add(0, primaryTarget.getStandingEyeHeight(), 0);
        Vec3d dir = targetPoint.subtract(origin).normalize();
        ...
        for (LivingEntity e : nearby) {
            ...
            e.damage(src, 16.0f); // buffed from 10 — was landing far too rarely to matter at the old value
